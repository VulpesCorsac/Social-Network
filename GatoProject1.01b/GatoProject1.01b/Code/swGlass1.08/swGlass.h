...
class Glass
	...
	public:
	   ...
	   ~Glass ();	
		...
		bool set_title (char* new_title) {return SetWindowText (hwnd (), new_title);}
...
bool Glass :: init (int sx, int sy, int style, HICON icon, HICON small_icon)
	...
	for (int i = 0; i < CLASS_NAME_LENGTH; i ++) 
		{
		class_name_ [i] = MIN_LETTER_FOR_NAME + rand () % (MAX_LETTER_FOR_NAME - 
														   MIN_LETTER_FOR_NAME);	
		}
...
bool Glass :: set_point (int x, int y, const swWinPixel& color)
	{
	SetPixel (dc (), x, y, RGB (color [R_COLOR_POS], color [G_COLOR_POS], color [B_COLOR_POS]));
...
bool use_handle_mouse () 
	{
	__GLASS_WND.add_msg (Swamp :: Message (WM_MOUSEMOVE, on_mouse));
	return true;
	}
