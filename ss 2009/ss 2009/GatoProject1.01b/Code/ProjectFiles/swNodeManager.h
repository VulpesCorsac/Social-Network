...
bool swNodeManager :: spreat (indx_t what) // what - номер объекта, который распространяет инфу
	...
	for (int i = 0; i < size_; i ++) // цикл проверки всех объектов
		{
		rand_zero_to_one = (float) (rand () % BIG_NUM_FOR_RAND) / 
									BIG_NUM_FOR_RAND;
			
		if (arcs_ [i * size_ + what]) //если есть связь между данным и 
									  //текущим (i-тым) объектом, то:
			{
			if (rand_zero_to_one < 1 - pow (SW_E, - KOEF_ARC_LENGTH / 
												  (float) arcs_ [i * size_ + what]/10))
...
bool swNodeManager :: test_all_coords (indx_t pos, pos_t& x, pos_t& y)
  ...
  x = - GRAPH_SQR * size_ + ((pos_t) (rand () % BIG_NUM_FOR_RAND) / BIG_NUM_FOR_RAND) 
                                                                              * area;
  y = - GRAPH_SQR * size_ + ((pos_t) (rand () % BIG_NUM_FOR_RAND) / BIG_NUM_FOR_RAND) 
                                                                              * area;
